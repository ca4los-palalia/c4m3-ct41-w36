...
@Repository
public class ProductoCostosRest extends RestMetaclas {
	...
	public CtrlRestService save(ProductoCostos itemObject) {
		...
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_SAVE), gson.toJson(itemObject));
	...
	public CtrlRestService delete(ProductoCostos itemObject) {
		CtrlRestService itemReturn = new CtrlRestService();
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_DELETE), gson.toJson(itemObject));
		if(!response.contains(ConstAtributos.ERROR_EXCEPTION))
	...
	public CtrlRestService getById(Long idFinder, Organizacion organizacion) {
		...
		CtrlRestService itemReturn = new CtrlRestService();
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_BY_ID), gson.toJson(tranferParam));
		if(!response.contains(ConstAtributos.ERROR_EXCEPTION)){
			itemReturn = createResponseToProductoCostos(response);
		} else
			itemReturn.setResponse(response);
	...
	public CtrlRestService getAll(Organizacion organizacion) {
		CtrlRestService itemReturn = new CtrlRestService();
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_ALL), gson.toJson(organizacion));
		if(!response.contains(ConstAtributos.ERROR_EXCEPTION)){
	...
	public CtrlRestService getByDescripcionAndProducto(String descripcion, Producto producto) {
		...
		CtrlRestService itemReturn = new CtrlRestService();
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_BY_DESCRIPCION_AND_PRODUCTO), gson.toJson(tranferParam));
		if(!response.contains(ConstAtributos.ERROR_EXCEPTION)){
			itemReturn = createResponseToListProductoCostos(response);
		} else
			itemReturn.setResponse(response);
	...
	public CtrlRestService getByProductoOrderMostRecentDate(Producto producto) {
		CtrlRestService itemReturn = new CtrlRestService();
		String response = procesarRestFULL((cfg.getValor() + ConstPath.MAP_PRODUCTO_COSTOS_BY_PRODUCTO_ORDER_MOST_RECENT_DATE), gson.toJson(producto));
		if(!response.contains(ConstAtributos.ERROR_EXCEPTION)){
			itemReturn = createResponseToListProductoCostos(response);
		} else
			itemReturn.setResponse(response);
